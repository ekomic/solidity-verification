...
/**
 * getTokensAfterRemove: Calculates the amount of `Eth` and `CH` tokens
...
export const getTokensAfterRemove = async (
  ...
  try {
    ...
    // Similarly we also maintain a ratio for the `CH` tokens, so here in our case
    // Ratio is -> (amount of CH tokens sent back to the user / CH Token reserve) = (LP tokens withdrawn) / (total supply of LP tokens)
    // Then (amount of CH tokens sent back to the user) = (CH token reserve * LP tokens withdrawn) / (total supply of LP tokens)
    const _removeEther = _ethBalance.mul(removeLPTokenWei).div(_totalSupply);
    const _removeCH = chrisTokenReserve
